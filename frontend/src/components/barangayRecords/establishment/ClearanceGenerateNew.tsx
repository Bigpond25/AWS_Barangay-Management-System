...
const ClearanceGenerateNew: React.FC = () => {
  ...
  const saveFileToServer = async (pdfBytes: Uint8Array, type: string) => {
      ...
      await establishmentService.attachNewClearance(Number(form.id), formData);
  ...
  const handlePrint = async () => {
    ...
    pdfForm.getTextField('applicant_name').updateAppearances(italicFont);
  ...
  const fetchEstablishment = async () => {
    try {
      ...
      const roomUnit = data.room_unit ?? "";
      const building = data.building ?? "";
      const no = data.no ?? "";
      const location = data.location ?? "";

      const address = roomUnit + " " + building + " " + no + " " + location;
    ...
    <div className="p-6">
      ...
      >
        ...
        <div className="mb-4">
          ...
          <p className="my-2">for the clearance to operate the business of{" "}</p>
          <div style={{ width: "100%"}}>
            <input
              name="business"
              disabled
              ...
              className="border border-black bg-[#d6dfec] focus:outline-none px-1 text-center w-full"
            />{" "}
          </div>
          <p className="text-center my-3">located at{" "}</p>
          <div style={{ width: "100%"}}>
            <input
              name="address"
              value={form.address}
        ...
        <div className="mt-12 flex justify-center">
          ...
          <div className="col-span-1 text-xs space-y-1 w-[33%]">
            <div>
              <div className="border border-red-100 p-1 w-[50%] relative ms-5">
                <p className="absolute top-[-7px] left-2 text-xs text-blue-300 font-bold">NEW</p>
                <p className="text-center text-blue-300 font-bold" style={{ fontSize: "30px" }}>2025</p>
              </div>
            </div>
            ...
            <div className="flex justify-center">
              <label className="font-semibold w-[50%]">OR #: </label>
              <input
                name="orNumber"
                value={form.orNumber}
                onChange={handleChange}
          ...
          <div className="col-span-1 text-center flex flex-col w-[33%]">
            <div>
              <p className="font-bold" style={{fontSize: '12px'}}>ELMER TIMOTHY J. LIGON</p>
...
export default ClearanceGenerateNew;
