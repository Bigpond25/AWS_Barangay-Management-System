...
import { FiPlus, FiEye, FiEdit, FiTrash2, FiX, FiMoreHorizontal } from 'react-icons/fi';
...
const Infrastructure: React.FC = () => {
  ...
  const fetchInfrastructures = async (page = 1, search = '') => {
    setIsLoading(true);
    try {
      const data = await infrastructureService.getInfrastructures({
        page,
        search,
      });
  ...
  useEffect(() => {
    const delayDebounce = setTimeout(() => {
      fetchInfrastructures(1, searchTerm);
    }, 400);
  ...
  const handlePageChange = (newPage: number) => {
    if (newPage !== pagination.current_page && newPage > 0 && newPage <= pagination.last_page) {
      fetchInfrastructures(newPage, searchTerm);
    }
  };
  ...
  // 🔹 Modal handlers
  const handleAddNew = () => {
    navigate(`/barangay-records/infrastructures/add`);
  };

  const handleDialogClose = () => {
    setIsDialogOpen(false);
    setNewInfrastructure({ businessName: '', owner: '', location: '' });
  };

  // 🔹 Save infrastructure (API POST request)
  const handleSave = async () => {
    if (!newInfrastructure.businessName.trim()) {
    ...
    try {
      setIsLoading(true);
      const response = await fetch(`${BASE_URL}/infrastructures`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          business_name: newInfrastructure.businessName,
          owner: newInfrastructure.owner,
          location: newInfrastructure.location,
        }),
      });

      if (!response.ok) throw new Error(`Failed to save infrastructure (${response.status})`);
      const data = await response.json();

      setInfrastructures([data, ...infrastructures]); // Prepend new record
      handleDialogClose();
    } catch (error) {
  ...
  const handleView = (infrastructure: InfrastructureSchema) => {
  ...
  <tbody className="bg-white divide-y divide-gray-200">
    {isLoading ? (
      ...
      infrastructures.map((infra) => (
        <tr key={infra.id} className="hover:bg-gray-50 transition">
          <td className="px-6 py-4 text-sm text-gray-700">{infra.id}</td>
          <td className="px-6 py-4 text-sm text-gray-700">{infra.date_of_application}</td>
          <td className="px-6 py-4 text-sm text-gray-700">{infra.type_of_project}</td>
          <td className="px-6 py-4 text-sm text-gray-700">{infra.classification}</td>
          <td className="px-6 py-4 text-sm text-gray-700">{infra.name_of_applicant}</td>
          <td className="px-6 py-4 text-sm text-gray-700">{infra.address_of_applicant}</td>
          <td className="px-6 py-4 text-sm text-gray-700">{infra.applicant_contact_no}</td>
          <td className="px-6 py-4 text-sm text-gray-700">{infra.applicants_representative}</td>
          <td className="px-6 py-4 text-sm text-gray-700">{infra.location_of_project}</td>
          <td className="px-6 py-4 text-sm text-gray-700">{infra.property_owner}</td>
      ...
      {isDialogOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-white/30 backdrop-blur-sm z-50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 relative">
            ...
            <h2 className="text-xl font-semibold mb-4 text-gray-800">Add New Infrastructure</h2>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  type="text"
                  value={newInfrastructure.businessName}
                  onChange={(e) =>
                    setNewInfrastructure({ ...newInfrastructure, businessName: e.target.value })
                  }
              ...
              <div>
                ...
                <input
                  type="text"
                  value={newInfrastructure.owner}
                  onChange={(e) =>
                    setNewInfrastructure({ ...newInfrastructure, owner: e.target.value })
                  }
                  className="mt-1 w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-smblue-400 focus:border-smblue-400 outline-none"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewInfrastructure({ ...newInfrastructure, location: e.target.value })
                  }
                  className="mt-1 w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-smblue-400 focus:border-smblue-400 outline-none"
                />
              </div>
