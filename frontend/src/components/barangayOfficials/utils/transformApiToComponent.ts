    ...
    position: official?.position === 'BARANGAY_CAPTAIN' ? 'Barangay Captain' :
      official?.position === 'KAGAWAD' ? 'Kagawad' :
        official?.position === 'BARANGAY_SECRETARY' ? 'Secretary' :
          official?.position,
    contact: official?.contact_number || official?.contact_number || 'N/A',
    term: official.term_start && official.term_end ? 
      `${new Date(official.term_start).getFullYear()} - ${new Date(official.term_end).getFullYear()}` : 
      'N/A',
    status: official?.status === 'ACTIVE' ? 'Active' :
      official?.status === 'INACTIVE' ? 'Inactive' :
        official?.status || 'N/A',
    committee: official.committee_assignment || official.committee || 'None',
    nationality: 'Filipino', // Default since not in API
    photo: official.profile_photo_url || official.profile_photo || 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-4.0.3&auto=format&fit=crop&w=150&q=80'
});