...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const logout = useCallback(async (): Promise<void> => {
    try {
      ...
      if (token) {
        try {
          const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://127.0.0.1:8000'}/api/auth/logout`, {
            method: 'POST',
            headers: {
      ...
      try {
        ...
        if (savedToken && savedUser) {
          try {
            ...
            const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://127.0.0.1:8000'}/api/auth/user`, {
              headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
